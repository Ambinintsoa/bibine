...
public class AnnonceRepositoryImpl implements AnnonceRepositoryCustom {
        ...
        public List<Statistique> countAllByModele() {
                ...
                AggregationResults<Statistique> results = mongoTemplate.aggregate(aggregation, "annonce",
                                Statistique.class);
        ...
        public double sumOfCommissions() {
                Aggregation aggregation = Aggregation.newAggregation(
                                Aggregation.match(Criteria.where("state").is(2)),
                                Aggregation.project()
                                                .andExpression("commission * 100 / prix").as("adjustedCommission"),
                                Aggregation.group()
                                                .sum("adjustedCommission").as("totalCommission"));
                ...
                return mappedResults.isEmpty() ? 0.0 : mappedResults.get(0).getTotalCommission();
        }
}
